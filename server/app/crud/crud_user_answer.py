...
class UserAnswerService :
    ...
    def save_answer(
        ...
        Args:
            ...
            user_id: ID пользователя (или guest_id)
            quiz_id: ID квиза
        ...
        # В реальном коде нужно передавать db сессию
    ...
    def _check_answer(
        ...
        В реальном коде нужно делать запрос к БД
        """
        # Здесь должна быть логика проверки через БД
        # Упрощенная версия для примера
        if answer_data.answer_id == 1 :  # Для теста
            return True, 10
        return False, 0
