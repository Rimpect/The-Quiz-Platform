...
for (const file of modules) {
  ...
  const jsFiles = glob.sync(file.replace(/\.module\.scss$/, '.js'), {cwd: root})
  ...
  const used = new Set();
  const scanFiles = jsFiles.length ? jsFiles : glob.sync('src/**/*.{js,jsx,ts,tsx}', {cwd: root, nodir: true});
  ...
  if (extra.length || unused.length) {
    ...
    if (unused.length) console.log(' defined not used:', unused.slice(0,20).join(', ') + (unused.length>20 ? ` (+${unused.length-20})` : ''));
    console.log('---');
  }
}
